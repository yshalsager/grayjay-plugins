import { content_thumbnails, plugin_icon_url } from '@lib/assets.js'
...
const PLATFORM = 'Mixlr'
const BASE_URL = 'https://mixlr.com'
const API_BASE = 'https://api.mixlr.com/v3'
const API_CDN_BASE = 'https://apicdn.mixlr.com/v3'
const DEFAULT_ICON = 'https://mixlr.com/favicon.ico'
...
source.getHome = function () {
  ...
  if (mode === HOME_MODES.SEARCH) {
    return new MixlrSearchPager(defaultSearchQuery(), selectedCategoryId(null), 'all', 1)
  }

  const categoryId = mode === HOME_MODES.CATEGORY ? homeCategoryId() : ''
  return new MixlrDiscoveryPager(categoryId, 1)
}

source.getSearchCapabilities = () => ({
  types: [Type.Feed.Mixed],
  sorts: [Type.Order.Chronological, Type.Order.Popularity],
  filters: [contentFilter(), categoryFilter(), statusFilter()]
})

source.searchSuggestions = function (query) {
  const q = normalize(query)
  const suggestions = ['radio', 'sports', 'music', 'talk', 'news', 'religion', 'church', 'jazz', 'culture', 'live']
  return suggestions.filter((item) => !q || normalize(item).indexOf(q) >= 0).slice(0, 10)
}
...
source.searchChannels = function (query) {
  if (!normalize(query)) {
    return new ArrayChannelPager([rootChannel()], DEFAULT_LIMIT)
  }
...
source.getChannelContents = function (url, _type, _order, filters) {
  if (isRootUrl(url)) {
    return source.search('', _type, _order, filters)
  }
...
source.searchChannelContents = function (url, query, _type, _order, filters) {
  if (isRootUrl(url)) {
    return source.search(query, _type, _order, filters)
  }
...
source.getContentDetails = function (url) {
  ...
  if (!parts) {
    throw new ScriptException('Unsupported Mixlr URL')
  }

  return eventToVideo(fetchEvent(parts.slug, parts.eventId), fetchChannel(parts.slug))
}

class MixlrDiscoveryPager extends VideoPager {
  constructor(categoryId = '', page = 1, items = null, offset = 0, sourceHasMore = false, excludeUrl = null) {
    const loaded = items ? { items, hasMore: sourceHasMore, nextPage: page } : loadDiscovery(categoryId, page, excludeUrl)
  ...
  nextPage() {
    ...
    const next = new MixlrDiscoveryPager(
      this.context.categoryId,
      this.context.page,
      remainingItems,
      remainingItems ? this.context.offset : 0,
      remainingItems ? this.context.sourceHasMore : false,
      this.context.excludeUrl
    )
    return apply_pager_state(this, next)
  }
}

class MixlrSearchPager extends VideoPager {
  constructor(query, categoryId = '', content = 'all', page = 1, items = null, offset = 0, sourceHasMore = false) {
    const loaded = items ? { items, hasMore: sourceHasMore, nextPage: page } : loadSearch(query, categoryId, content, page)
    const results = loaded.items.slice(offset, offset + DEFAULT_LIMIT)
    const nextOffset = offset + DEFAULT_LIMIT
  ...
  nextPage() {
    const remainingItems = this.context.offset < this.context.items.length ? this.context.items : null
    const next = new MixlrSearchPager(
      this.context.query,
      this.context.categoryId,
...
class MixlrChannelPager extends ChannelPager {
  constructor(query, page = 1, items = null, offset = 0, sourceHasMore = false) {
    const loaded = items ? { items, hasMore: sourceHasMore, nextPage: page } : loadChannelSearch(query, page)
  ...
  nextPage() {
    const remainingItems = this.context.offset < this.context.items.length ? this.context.items : null
    const next = new MixlrChannelPager(
      this.context.query,
      this.context.page,
      remainingItems,
      remainingItems ? this.context.offset : 0,
...
function loadSearch(query, categoryId, content, page) {
  ...
  if (includeChannels) {
    ...
    for (const slug of parseChannelSlugs(html)) {
      ...
      if (video && (!categoryId || video.categoryId === categoryId)) {
        items.push(video)
      }
    }
    hasMore = hasMore || hasNextPage(html)
...
function loadChannelSearch(query, page) {
  const html = fetchPage(searchUrl(query, 'Channel', page))
  return {
    items: parseChannelSlugs(html)
      .map((slug) => safeChannel(slug))
      .filter(Boolean),
    hasMore: hasNextPage(html),
    nextPage: page + 1
  }
...
function eventLinksToVideos(links, excludeUrl = null, categoryId = '') {
  ...
  for (const link of links) {
    ...
    if (!video || video.url === excludeUrl || (categoryId && video.categoryId !== categoryId)) {
      continue
    }
    videos.push(video)
  }
...
function fetchChannel(slug) {
  ...
  return cached('channelCache', cleanSlug, () =>
    normalizeChannel(cleanSlug, get_json(`${API_CDN_BASE}/channel_view/${cleanSlug}`, DEFAULT_HEADERS, false))
  )
}
...
function fetchEvent(slug, eventId) {
  ...
  if (!cleanEventId) {
    throw new ScriptException('Mixlr event id not found')
  }

  return cached('eventCache', `${cleanSlug}:${cleanEventId}`, () =>
    normalizeEvent(cleanSlug, cleanEventId, get_json(`${API_BASE}/channels/${cleanSlug}/events/${cleanEventId}`, DEFAULT_HEADERS, false))
  )
}
...
function channelToPlatformChannel(channel) {
  return grayjay.channel(`channel-${channel.slug}`, {
    ...
    urlAlternatives: [publicChannelUrl(channel.slug), `${BASE_URL}/${channel.slug}`, channel.legacyUrl].filter(Boolean)
  })
}
...
function rootChannel() {
  return grayjay.channel('root', {
    ...
    url: BASE_URL,
    urlAlternatives: [BASE_URL, `${BASE_URL}/live/popular`, `${BASE_URL}/search`]
  })
}
...
function channelCurrentVideo(channel) {
  ...
  if (!broadcast.live || !broadcast.streamUrl) {
    return null
  }
...
function eventToVideo(event, channel) {
  ...
  if (!broadcast.live || !broadcast.streamUrl) {
    throw new ScriptException('Mixlr event is not currently live')
  }
...
function parseEventLinks(html) {
  ...
  while ((match = regex.exec(String(html ?? ''))) !== null) {
    ...
    if (!seen[key]) {
      seen[key] = true
      links.push({ slug, eventId, url: decode_html(match[1]) })
    }
  }
  return links
}
...
function eventPartsFromUrl(url) {
  ...
  if (!match?.[1] || !match?.[2] || !validSlug(match[1])) {
    return null
  }
...
function assertSlug(slug) {
  const value = String(slug ?? '')
    ...
    .toLowerCase()
  if (!validSlug(value)) {
    throw new ScriptException('Mixlr channel not found')
  }
  return value
}
...
function videoMatches(video, query) {
  ...
  return (
    !q ||
    normalize(video.name).indexOf(q) >= 0 ||
    normalize(video.description).indexOf(q) >= 0 ||
    normalize(video.author?.name).indexOf(q) >= 0
  )
}
...
function homeCategoryId() {
  const index = Number(_settings.homeCategory ?? 0)
...
function contentFilter() {
  return {
    ...
    isMultiSelect: false,
    filters: [
      { id: 'all', name: 'Live events and channels', value: 'all' },
      { id: 'events', name: 'Live events', value: 'events' },
      { id: 'channels', name: 'Live channels', value: 'channels' }
    ]
  }
}
...
function statusFilter() {
  return {
    ...
    name: 'Status',
    isMultiSelect: false,
    filters: [{ id: 'live', name: 'Live only', value: 'live' }]
  }
}
