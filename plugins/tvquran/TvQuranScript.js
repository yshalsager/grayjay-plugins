...
function pluginAsset(path) {
  ...
  const base = String(_config.sourceUrl ?? '').replace(/\/[^/]*$/, '/')
  return base ? `${base}${url.replace(/^\.\//, '')}` : url
}

function iconUrl() {
  return _config.absoluteIconUrl ?? pluginAsset(_config.iconUrl ?? DEFAULT_ICON)
}

function staticThumbnails() {
  return new Thumbnails([new Thumbnail(iconUrl(), 0)])
}
...
class ArrayVideoPager extends VideoPager {
  ...
  nextPage() {
    const next = new ArrayVideoPager(this.context.items, this.context.limit, this.context.offset)
    this.results = next.results
    this.hasMore = next.hasMore
    this.context = next.context
    return this
...
class ArrayPlaylistPager extends PlaylistPager {
  constructor(items, limit, offset = 0) {
    const results = items.slice(offset, offset + limit)
    super(results, offset + limit < items.length, { items, limit, offset: offset + limit })
  }
  ...
  nextPage() {
    const next = new ArrayPlaylistPager(this.context.items, this.context.limit, this.context.offset)
    this.results = next.results
    this.hasMore = next.hasMore
    this.context = next.context
    return this
...
function selectionToVideo(selection) {
  const details = new PlatformVideoDetails({
    id: new PlatformID(PLATFORM, String(selection.id), _config.id),
    name: selection.title,
    thumbnails: selection.thumbnail ? new Thumbnails([new Thumbnail(selection.thumbnail, 0)]) : staticThumbnails(),
    author: new PlatformAuthorLink(
      new PlatformID(PLATFORM, normalize(selection.author).replace(/[^a-z0-9]+/g, '-'), _config.id),
...
function recitationToVideo(recitation) {
  const details = new PlatformVideoDetails({
    id: new PlatformID(PLATFORM, `recitation-${recitation.id}`, _config.id),
    name: `${recitation.title} - ${recitation.reciter.name}`,
    thumbnails: recitation.thumbnail ? new Thumbnails([new Thumbnail(recitation.thumbnail, 0)]) : staticThumbnails(),
...
function tvVideoToNested(video) {
  const nested = new PlatformNestedMediaContent({
    id: new PlatformID(PLATFORM, `video-${video.id}`, _config.id),
    name: video.title,
    author: new PlatformAuthorLink(
      new PlatformID(PLATFORM, 'tvquran-videos', _config.id),
      'tvQuran',
      videoCategoryChannelUrl(video.videoType),
      iconUrl()
    ),
    datetime: 0,
    ...
    contentThumbnails: video.thumbnail ? new Thumbnails([new Thumbnail(video.thumbnail, 0)]) : staticThumbnails()
  })
...
function categoryToChannel(category, enrich = false) {
  ...
  return new PlatformChannel({
    id: new PlatformID(PLATFORM, `category-${category.id}`, _config.id),
...
function videoCategoryToChannel(category, enrich = false) {
  ...
  return new PlatformChannel({
    id: new PlatformID(PLATFORM, `video-category-${category.id}`, _config.id),
...
function collectionToChannel(collection) {
  return new PlatformChannel({
    id: new PlatformID(PLATFORM, `collection-${collection.id}`, _config.id),
...
function rootChannel() {
  return new PlatformChannel({
    id: new PlatformID(PLATFORM, 'root', _config.id),
    name: 'tvQuran',
    thumbnail: iconUrl(),
    banner: iconUrl(),
    subscribers: 0,
...
function reciterToChannel(reciter) {
  return new PlatformChannel({
    id: new PlatformID(PLATFORM, `reciter-${reciter.id}`, _config.id),
    name: reciter.name,
    thumbnail: reciter.thumbnail || iconUrl(),
...
function reciterToAuthor(reciter) {
  return new PlatformAuthorLink(
    new PlatformID(PLATFORM, `reciter-${reciter.id}`, _config.id),
    reciter.name,
    reciterChannelUrl(reciter.id, reciter.slug),
    reciter.thumbnail || iconUrl()
  )
}
...
function categoryToPlaylist(category, enrich = false) {
  ...
  return new PlatformPlaylist({
    id: new PlatformID(PLATFORM, `playlist-category-${category.id}`, _config.id),
    name: categoryName(category),
...
function collectionToPlaylist(collection) {
  ...
  return new PlatformPlaylist({
    id: new PlatformID(PLATFORM, `playlist-collection-${collection.id}`, _config.id),
    name: collection.title,
    thumbnails: new Thumbnails([new Thumbnail(thumbnail, 0)]),
...
function tvQuranAuthor(id, name, url, thumbnail = iconUrl()) {
  return new PlatformAuthorLink(new PlatformID(PLATFORM, id, _config.id), name, url, thumbnail || iconUrl())
}
...
function pruneCache(cacheName) {
  const limit = CACHE_LIMITS[cacheName] ?? DEFAULT_LIMIT
  const order = state.cacheOrder?.[cacheName]
  ...
  while (order.length > limit) {
    delete state[cacheName][order.shift()]
  }
}
