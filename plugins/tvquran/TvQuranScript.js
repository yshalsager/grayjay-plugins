(function() {
	...
	function last_match(value, regex) {
		let result = null;
		let match;
		regex.lastIndex = 0;
		while ((match = regex.exec(String(value ?? ""))) !== null) result = match[1];
		return result;
	}
	...
	var PLATFORM = "tvQuran";
	var BASE_URL = "https://tvquran.com";
	var DEFAULT_LIMIT = 20;
	var RECITATION_SCAN_PER_PAGE = 4;
	var DEFAULT_ICON = "./TvQuranIcon.png";
	var CACHE_LIMITS = {
		selectionCache: 120,
		reciterCache: 300,
		recitationCache: 600,
	...
	var CATEGORIES = [
		...
		},
		{
			...
			name: {
				ar: "تلاوات نادرة",
				en: "Rare Recitations",
				de: "Seltene Rezitationen"
			},
			slug: {
				ar: "%D8%AA%D9%84%D8%A7%D9%88%D8%A7%D8%AA-%D9%86%D8%A7%D8%AF%D8%B1%D8%A9",
				en: "rare-recitations",
				de: "seltene-rezitationen"
			}
		},
		...
		},
		{
			...
			slug: {
				ar: "%D8%A7%D9%84%D8%B1%D9%82%D9%8A%D8%A9-%D8%A7%D9%84%D8%B4%D8%B1%D8%B9%D9%8A%D8%A9",
				en: "ruqyah",
				de: "ruqyah"
			}
		},
		{
			...
			slug: {
				ar: "%D8%A3%D8%B0%D9%83%D8%A7%D8%B1-%D8%A7%D9%84%D9%8A%D9%88%D9%85",
				en: "daily-dhikr",
				de: "taglich-dhikr"
			}
		},
		{
			...
			slug: {
				ar: "%D8%A3%D8%AF%D8%B9%D9%8A%D8%A9-%D9%85%D8%AE%D8%AA%D8%A7%D8%B1%D8%A9",
				en: "dua",
				de: "dua"
			}
		},
		...
		},
		{
			...
			name: {
				ar: "قرآنا عجبا",
				en: "Amazing Quran",
				de: "Wunderbarer Quran"
			},
			slug: {
				ar: "%D9%82%D8%B1%D8%A2%D9%86%D8%A7-%D8%B9%D8%AC%D8%A8%D8%A7",
				en: "amazing-quran",
				de: "wunderbarer-quran"
			}
		},
	...
	var VIDEO_CATEGORY_DESCRIPTIONS = {
		videos: {
	...
	var VIDEO_CATEGORIES = [
		{
			...
			slug: {
				ar: "%D9%81%D9%8A%D8%AF%D9%8A%D9%88-%D8%A7%D9%84%D8%AA%D9%84%D8%A7%D9%88%D8%A7%D8%AA",
				en: "videos",
				de: "videos"
			}
		},
		...
		},
		{
			...
			slug: {
				ar: "%D8%A7%D9%84%D8%A8%D8%AB-%D8%A7%D9%84%D9%85%D8%A8%D8%A7%D8%B4%D8%B1",
				en: "live-broadcast",
				de: "live-sendung"
			}
		}
	...
	source.getHome = function() {
		...
		if (homeMode === "1") return tvVideosPager("videos");
		if (homeMode === "2") return tvVideosPager("live-videos");
		if (homeMode === "3") return new TvQuranReciterHomePager();
		if (homeMode === "4") return tvVideosPager("prayer-videos");
		const category = CATEGORIES[Number(_settings.homeCategory ?? 0)] ?? CATEGORIES[0];
		return new TvQuranPager(categoryUrl(category.id, 1, homeSelectionView()), {
			categoryId: category.id,
			page: 2,
			view: homeSelectionView()
		});
	};
	...
	source.searchSuggestions = function(query) {
		...
		if (q.length >= 2) try {
			...
			suggestions.push(...reciters.filter((reciter) => reciter.collections?.length).flatMap((reciter) => reciter.collections.map((collection) => collection.title)));
		} catch (e) {
	...
	source.search = function(query, type, order, filters) {
		if (!normalize(query) && !hasSearchFilters(filters)) return source.getHome();
	...
	source.searchChannels = function(query) {
		...
		const videoChannels = VIDEO_CATEGORIES.filter((category) => !q || normalize(videoCategoryName(category)).indexOf(q) >= 0).map(videoCategoryToChannel);
	...
	source.isChannelUrl = function(url) {
		return REGEX.ROOT_CHANNEL.test(url) || REGEX.CATEGORY_CHANNEL.test(url) || REGEX.WEB_CATEGORY.test(url) || REGEX.VIDEO_CHANNEL.test(url) || REGEX.WEB_VIDEO_CATEGORY.test(url) || REGEX.RECITER_CHANNEL.test(url) || REGEX.WEB_RECITER.test(url) || REGEX.COLLECTION_CHANNEL.test(url) || REGEX.WEB_COLLECTION.test(url);
	};
	source.getChannel = function(url) {
		if (REGEX.ROOT_CHANNEL.test(url)) return rootChannel();
		if (REGEX.CATEGORY_CHANNEL.test(url) || REGEX.WEB_CATEGORY.test(url)) return categoryToChannel(getCategoryFromUrl(url), true);
		if (REGEX.VIDEO_CHANNEL.test(url) || REGEX.WEB_VIDEO_CATEGORY.test(url)) return videoCategoryToChannel(getVideoCategoryFromUrl(url), true);
		if (REGEX.COLLECTION_CHANNEL.test(url) || REGEX.WEB_COLLECTION.test(url)) return collectionToChannel(getCollectionFromUrl(url));
		return reciterToChannel(getReciterFromUrl(url));
	};
	source.isPlaylistUrl = function(url) {
		return REGEX.CATEGORY_PLAYLIST.test(url) || REGEX.WEB_CATEGORY.test(url) || REGEX.COLLECTION_PLAYLIST.test(url) || REGEX.COLLECTION_CHANNEL.test(url) || REGEX.WEB_COLLECTION.test(url);
	};
	source.getPlaylist = function(url) {
		return getTvQuranPlaylist(url);
	};
	source.getChannelPlaylists = function(url) {
		if (REGEX.ROOT_CHANNEL.test(url)) return new ArrayPlaylistPager(tvQuranPlaylists("", Type.Order.Chronological, null), DEFAULT_LIMIT);
		if (REGEX.CATEGORY_CHANNEL.test(url) || REGEX.WEB_CATEGORY.test(url)) return new ArrayPlaylistPager([categoryToPlaylist(getCategoryFromUrl(url), true)], DEFAULT_LIMIT);
		if (REGEX.VIDEO_CHANNEL.test(url) || REGEX.WEB_VIDEO_CATEGORY.test(url)) return new ArrayPlaylistPager([], DEFAULT_LIMIT);
		if (REGEX.COLLECTION_CHANNEL.test(url) || REGEX.WEB_COLLECTION.test(url)) return new ArrayPlaylistPager([collectionToPlaylist(getCollectionFromUrl(url))], DEFAULT_LIMIT);
	...
	source.peekChannelContents = function(url, _type) {
		if (REGEX.ROOT_CHANNEL.test(url)) return source.getHome().results.slice(0, 6);
		if (REGEX.CATEGORY_CHANNEL.test(url) || REGEX.WEB_CATEGORY.test(url)) return parseSelections(callHtml(categoryUrl(getCategoryFromUrl(url).id, 1, homeSelectionView()))).slice(0, 6).map(selectionToVideo);
		if (REGEX.VIDEO_CHANNEL.test(url) || REGEX.WEB_VIDEO_CATEGORY.test(url)) {
			const category = getVideoCategoryFromUrl(url);
			return parseTvVideos(callHtml(videoCategoryUrl(category.type, 1)), category.type).slice(0, 6).map(tvVideoToNested);
		}
		if (REGEX.COLLECTION_CHANNEL.test(url) || REGEX.WEB_COLLECTION.test(url)) return collectionContentVideos(getCollectionFromUrl(url), "", null).slice(0, 6);
	...
	source.searchChannelContents = function(channelUrlValue, query, type, order, filters) {
		if (REGEX.ROOT_CHANNEL.test(channelUrlValue)) return source.search(query, type, order, filters);
		const q = normalize(query);
		if (REGEX.CATEGORY_CHANNEL.test(channelUrlValue) || REGEX.WEB_CATEGORY.test(channelUrlValue)) return new TvQuranChannelSearchPager("category", getCategoryFromUrl(channelUrlValue).id, query, order, filters);
		if (REGEX.VIDEO_CHANNEL.test(channelUrlValue) || REGEX.WEB_VIDEO_CATEGORY.test(channelUrlValue)) return new TvQuranChannelSearchPager("video", getVideoCategoryFromUrl(channelUrlValue).type, query, order, filters);
		if (REGEX.COLLECTION_CHANNEL.test(channelUrlValue) || REGEX.WEB_COLLECTION.test(channelUrlValue)) return new ArrayVideoPager(collectionContentVideos(getCollectionFromUrl(channelUrlValue), q, filters), DEFAULT_LIMIT);
		return new ArrayVideoPager(reciterContentVideos(getReciterFromUrl(channelUrlValue), q, filters), DEFAULT_LIMIT);
	};
	source.getChannelContents = function(url, type, order, filters) {
		if (REGEX.ROOT_CHANNEL.test(url)) return source.getHome();
		if (REGEX.CATEGORY_CHANNEL.test(url) || REGEX.WEB_CATEGORY.test(url)) {
			const category = getCategoryFromUrl(url);
			const view = selectionViewFromOrder(order);
			return new TvQuranPager(categoryUrl(category.id, 1, view), {
				categoryId: category.id,
				page: 2,
				view
			});
		}
		if (REGEX.VIDEO_CHANNEL.test(url) || REGEX.WEB_VIDEO_CATEGORY.test(url)) return tvVideosPager(getVideoCategoryFromUrl(url).type);
		if (REGEX.COLLECTION_CHANNEL.test(url) || REGEX.WEB_COLLECTION.test(url)) return new ArrayVideoPager(collectionContentVideos(getCollectionFromUrl(url), "", filters), DEFAULT_LIMIT);
		return new ArrayVideoPager(reciterContentVideos(getReciterFromUrl(url), "", filters), DEFAULT_LIMIT);
	};
	source.isContentDetailsUrl = function(url) {
		return REGEX.SELECTION.test(url) || REGEX.WEB_SELECTION.test(url) || REGEX.RECITATION.test(url) || REGEX.WEB_RECITATION.test(url) || REGEX.VIDEO.test(url) || REGEX.WEB_VIDEO.test(url);
	};
	source.getContentDetails = function(url) {
		if (REGEX.VIDEO.test(url) || REGEX.WEB_VIDEO.test(url)) return getTvVideoDetails(extractTvVideoId(url));
		if (REGEX.RECITATION.test(url) || REGEX.WEB_RECITATION.test(url)) return getRecitationDetails(extractRecitationId(url));
		const id = extractSelectionId(url);
		if (!id) throw new ScriptException("Unsupported tvQuran URL");
		if (state.selectionCache[id]) return selectionToVideo(state.selectionCache[id]);
		const selections = parseSelections(callHtml(`${BASE_URL}/${language()}/selection/${id}`));
		const selection = selections.find((item) => String(item.id) === String(id)) ?? selections[0];
		if (!selection) throw new ScriptException("Selection not found");
		cacheSet("selectionCache", id, selection);
		return selectionToVideo(selection);
	};
	var TvQuranPager = class extends VideoPager {
	...
	var TvQuranSearchPager = class TvQuranSearchPager extends VideoPager {
		constructor(query, order, filters, page = 1) {
			const pageResult = searchPage(query, order, filters, page);
			super(pageResult.videos, pageResult.hasMore, {
				query,
				order,
				filters,
				page: page + 1
	...
	var TvQuranChannelSearchPager = class TvQuranChannelSearchPager extends VideoPager {
		...
		nextPage() {
			const next = new TvQuranChannelSearchPager(this.context.kind, this.context.value, this.context.query, this.context.order, this.context.filters, this.context.page);
			return apply_pager_state(this, next);
		}
	};
	var ArrayVideoPager = array_pager_class(VideoPager);
	var ArrayPlaylistPager = array_pager_class(PlaylistPager);
	var TvQuranChannelPager = class TvQuranChannelPager extends ChannelPager {
		constructor(query, page = 1, categories = []) {
			...
			super(channels, hasLoadMore(html), {
				query,
				page: page + 1
			});
		}
	...
	var TvQuranVideoPager = class TvQuranVideoPager extends VideoPager {
	...
	var TvQuranReciterHomePager = class TvQuranReciterHomePager extends VideoPager {
		constructor(page = 1) {
			const pageResult = reciterSearchPage("", page);
			const videos = [];
			for (const reciter of pageResult.reciters.slice(0, RECITATION_SCAN_PER_PAGE)) videos.push(...reciterContentVideos(reciter, "", null).slice(0, 3));
	...
	function parseSelections(html) {
		...
		while ((match = articleRegex.exec(html)) !== null) {
			...
			if (!audioUrl || !/\/[a-z]{2}\/selection\//.test(article)) continue;
			const title = cleanText(extractFirst(article, /<h3[^>]*>([\s\S]*?)<\/h3>/) ?? `Selection ${shareId}`);
			const h4 = extractFirst(article, /<h4[^>]*>([\s\S]*?)<\/h4>/) ?? "";
			const author = cleanText(extractFirst(h4, /<a[^>]*title=["']([^"']+)["'][^>]*>/) ?? extractFirst(h4, /<a[^>]*>([\s\S]*?)<\/a>/) ?? "tvQuran");
			...
			const selection = {
				...
				audioUrl,
				views,
				pageUrl: absolutize(extractFirst(article, /data-url=["']([^"']*\/[a-z]{2}\/selection\/\d+)["']/)) ?? `${BASE_URL}/${language()}/selection/${shareId}`
			};
	...
	function parseReciterCards(html) {
		const reciters = [];
		const itemRegex = /<a[^>]*href=["']([^"']*\/[a-z]{2}\/scholar\/(\d+)\/profile\/([^"']+))["'][^>]*class=["'][^"']*sheikh-item[^"']*["'][\s\S]*?<\/a>/g;
		let match;
		while ((match = itemRegex.exec(html)) !== null) {
			...
			const reciter = {
				id: String(match[2]),
				slug: match[3],
				name: cleanText(extractFirst(item, /<h2[^>]*>([\s\S]*?)<\/h2>/) ?? extractFirst(item, /<img[^>]*alt=["']([^"']+)["']/) ?? `Reciter ${match[2]}`),
	...
	function parseReciterProfile(html, fallbackUrl) {
		...
		let reciter = cacheReciter({
			...
			slug: reciterMatch?.[2] ?? "",
			name: cleanText(extractFirst(details, /<h3[^>]*>([\s\S]*?)<\/h3>/) ?? extractFirst(html, /data-title=["']([^"']+)["']/) ?? "tvQuran Reciter"),
		...
		if (profileCollections.collections.length) reciter = cacheReciter({
			...reciter,
		...
		const recitations = profileCollections.recitations.length ? profileCollections.recitations.map((recitation) => ({
			...recitation,
			reciter
	...
	function parseReciterProfileCollections(html, reciter) {
		...
		while ((match = titleRegex.exec(html)) !== null) {
			...
			const title = cleanText(extractFirst(section, /<h2[^>]*>([\s\S]*?)<\/h2>/) ?? extractFirst(section, /data-title=["']([^"']+)["']/) ?? "");
			...
			recitations.push(...parseRecitations(section, reciter).map((recitation) => ({
				...recitation,
		...
		return {
			...
			recitations: dedupeById(recitations)
		};
	}
	function parseCollectionPage(html, fallbackUrl) {
		...
		const title = cleanText(extractFirst(html, /<div class=["']moshaf-title["'][\s\S]*?<h2[^>]*>([\s\S]*?)<\/h2>/) ?? extractFirst(html, /<h2[^>]*>([\s\S]*?)<\/h2>/) ?? extractFirst(html, /data-title=["']([^"']+)["']/) ?? `tvQuran Collection ${match?.[1] ?? ""}`);
		const reciterUrl = absolutize(extractFirst(html, /<a[^>]*href=["']([^"']*\/[a-z]{2}\/scholar\/\d+\/profile\/[^"']+)["'][^>]*title=["']([^"']+)["']/)) ?? canonical;
		const reciter = cacheReciter({
			...
			name: cleanText(extractFirst(html, /<a[^>]*href=["'][^"']*\/scholar\/\d+\/profile\/[^"']+["'][^>]*title=["']([^"']+)["']/) ?? title.replace(/\s+-\s+.*$/, "") ?? "tvQuran Reciter"),
	...
	function parseRecitations(html, reciter) {
		...
		while ((match = articleRegex.exec(html)) !== null) {
			...
			const title = cleanText(extractFirst(article, /<h4[^>]*>\s*<span>([\s\S]*?)<\/span>\s*<\/h4>/) ?? extractFirst(article, /data-title=["']([^"']+)["']/) ?? `Recitation ${id}`);
			const surahNumber = Number(cleanText(extractFirst(article, /<span[^>]*class=["'][^"']*sorah-num[^"']*["'][^>]*>([\s\S]*?)<\/span>/))) || 0;
			const views = Number((extractFirst(article, /fa-play[\s\S]*?<span[^>]*>([\d\s]+)<\/span>/) ?? "0").replace(/\s+/g, "")) || 0;
			const recitation = {
				id: String(id),
				playerId: match[1],
				...
				audioUrl,
				views,
				pageUrl: absolutize(extractFirst(article, /data-url=["']([^"']*\/[a-z]{2}\/recitation\/\d+)["']/)) ?? `${BASE_URL}/${language()}/recitation/${id}`
			};
	...
	function parseTvVideos(html, videoType) {
		...
		while ((match = articleRegex.exec(html)) !== null) {
			const article = match[0];
			const href = extractFirst(article, /<a[^>]*href=["']([^"']*\/[a-z]{2}\/video\/\d+[^"']*)["'][^>]*class=["'][^"']*vid-thumb/) ?? extractFirst(article, /<h3[^>]*>[\s\S]*?<a[^>]*href=["']([^"']*\/[a-z]{2}\/video\/\d+[^"']*)["']/);
			const id = extractFirst(href, /\/[a-z]{2}\/video\/(\d+)/);
			if (!id) continue;
			const title = cleanText(extractFirst(article, /<h3[^>]*>[\s\S]*?<a[^>]*title=["']([^"']+)["']/) ?? extractFirst(article, /<img[^>]*alt=["']([^"']+)["']/) ?? extractFirst(article, /<h3[^>]*>([\s\S]*?)<\/h3>/) ?? `Video ${id}`);
			const thumbnail = absolutize(extractFirst(article, /<img[^>]*src=["']([^"']+)["']/)) ?? youtubeThumbnail(extractYoutubeId(article)) ?? iconUrl();
			const description = cleanText(extractFirst(article, /<p[^>]*class=["'][^"']*mt-?15[^"']*["'][^>]*>([\s\S]*?)<\/p>/) ?? "");
			const views = Number((extractFirst(article, /fa-play[\s\S]*?<span[^>]*>([\d\s]+)<\/span>/) ?? "0").replace(/\s+/g, "")) || 0;
			const durationText = cleanText(extractFirst(article, /<span[^>]*class=["'][^"']*vid-duration[^"']*["'][^>]*>\s*<strong>([\s\S]*?)<\/strong>/) ?? "");
	...
	function selectionToVideo(selection) {
		const details = grayjay.video(selection.id, {
			...
			author: grayjay.author(normalize(selection.author).replace(/[^a-z0-9]+/g, "-"), selection.author || "tvQuran", selection.authorUrl || BASE_URL, selection.thumbnail || iconUrl()),
			uploadDate: 0,
			duration: 0,
			...
			video: audio_source_descriptor({
				name: "MP3 Audio",
				url: selection.audioUrl,
				language: "ar"
			}),
			shareUrl: selection.pageUrl
		});
		details.getContentRecommendations = function() {
			return new TvQuranPager(`${BASE_URL}/${language()}/search?query=${encodeURIComponent(selection.author || selection.category || selection.title)}`, {});
		};
		return details;
	}
	function recitationToVideo(recitation) {
		...
		details.getContentRecommendations = function() {
			return new ArrayVideoPager(reciterContentVideos(recitation.reciter, "").filter((video) => video.url !== details.url), DEFAULT_LIMIT);
		};
		return details;
	}
	...
	function getRecitationDetails(id) {
		if (!id) throw new ScriptException("Recitation not found");
		if (state.recitationCache[id]) return recitationToVideo(state.recitationCache[id]);
		const recitation = parseReciterProfile(callHtml(`${BASE_URL}/${language()}/recitation/${id}`), `${BASE_URL}/${language()}/recitation/${id}`).recitations.find((item) => String(item.id) === String(id));
		if (!recitation) throw new ScriptException("Recitation not found");
		return recitationToVideo(recitation);
	}
	function getTvVideoDetails(id) {
		if (!id) throw new ScriptException("tvQuran video not found");
		const html = callHtml(`${BASE_URL}/${language()}/video/${id}`);
		const canonical = extractFirst(html, /<link[^>]*rel=["']canonical["'][^>]*href=["']([^"']+)["']/) ?? `${BASE_URL}/${language()}/video/${id}`;
		const youtubeId = extractYoutubeId(html);
		return tvVideoToNested({
			id: String(id),
			title: cleanText(extractFirst(html, /<div class=["']l-box["'][\s\S]*?<h2[^>]*>([\s\S]*?)<\/h2>/) ?? extractFirst(html, /data-title=["']([^"']+)["']/) ?? `Video ${id}`),
			thumbnail: youtubeThumbnail(youtubeId) ?? iconUrl(),
			description: cleanText(extractFirst(html, /<meta name=["']description["'][^>]*content=["']([^"']+)["']/) ?? extractFirst(html, /<p[^>]*class=["'][^"']*mtb-15[^"']*["'][^>]*>([\s\S]*?)<\/p>/) ?? ""),
	...
	function getCategoryFromUrl(url) {
		const id = (url.match(REGEX.CATEGORY_CHANNEL) ?? url.match(REGEX.WEB_CATEGORY))?.[1];
		const category = CATEGORIES.find((c) => String(c.id) === String(id));
		if (!category) throw new ScriptException("tvQuran category not found");
		return category;
	}
	function getVideoCategoryFromUrl(url) {
		const value = (url.match(REGEX.VIDEO_CHANNEL) ?? url.match(REGEX.WEB_VIDEO_CATEGORY))?.[1];
		const category = VIDEO_CATEGORIES.find((c) => c.type === value || String(c.id) === String(value));
		if (!category) throw new ScriptException("tvQuran video category not found");
		return category;
	}
	function getReciterFromUrl(url) {
		const match = url.match(REGEX.RECITER_CHANNEL) ?? url.match(REGEX.WEB_RECITER);
		const id = match?.[1];
		if (!id) throw new ScriptException("tvQuran reciter not found");
		if (state.reciterCache[id]) return state.reciterCache[id];
		return parseReciterProfile(callHtml(reciterProfileUrl(id, match?.[2], urlLanguage(url))), reciterProfileUrl(id, match?.[2], urlLanguage(url))).reciter;
	}
	function getCollectionFromUrl(url) {
		const match = url.match(REGEX.COLLECTION_CHANNEL) ?? url.match(REGEX.WEB_COLLECTION);
		const id = match?.[1];
		if (!id) throw new ScriptException("tvQuran collection not found");
		return parseCollectionPage(callHtml(collectionPageUrl(id, match?.[2], urlLanguage(url))), collectionPageUrl(id, match?.[2], urlLanguage(url)));
	}
	...
	function tvQuranPlaylists(query, order, filters) {
		const q = normalize(query);
		const playlists = [];
		if (contentAllowed(filters, "selections")) playlists.push(...CATEGORIES.filter((category) => categoryMatchesFilters(category, filters)).map(categoryToPlaylist));
		if (contentAllowed(filters, "recitations")) playlists.push(...searchReciterCollectionPlaylists(query, filters));
	...
	function categoryToPlaylist(category, enrich = false) {
		const thumbnail = categoryChannelMetadata("selection", category, enrich).thumbnail || iconUrl();
	...
	function reciterCollectionPlaylists(reciter) {
		...
		return enrichedReciter.collections.map((collection) => collectionToPlaylist({
			...collection,
			reciter: enrichedReciter,
			recitations: (state.reciterContentsCache[enrichedReciter.id] ?? []).filter((recitation) => String(recitation.collectionId) === String(collection.id))
		}));
	}
	function searchReciterCollectionPlaylists(query, filters) {
		const selectedReciters = selectedFilterValues(filters, "reciter");
		const reciters = [];
		if (selectedReciters.length) for (const value of selectedReciters) {
			const reciter = resolveReciterFilter(value);
			if (reciter) reciters.push(reciter);
		}
		else {
			const q = normalize(query);
			const cached = Object.values(state.reciterCache ?? {}).filter((reciter) => reciter.collections?.length);
			reciters.push(...cached);
			if (q || cached.length < RECITATION_SCAN_PER_PAGE) try {
	...
	function reciterWithCollections(reciter) {
		if (reciter.collections?.length) return reciter;
		try {
	...
	function getTvQuranPlaylist(url) {
		if (REGEX.CATEGORY_PLAYLIST.test(url) || REGEX.WEB_CATEGORY.test(url)) return categoryToPlaylistDetails(getPlaylistCategoryFromUrl(url), Type.Order.Chronological);
		if (REGEX.COLLECTION_PLAYLIST.test(url) || REGEX.COLLECTION_CHANNEL.test(url) || REGEX.WEB_COLLECTION.test(url)) return collectionToPlaylistDetails(getPlaylistCollectionFromUrl(url));
	...
	function getPlaylistCategoryFromUrl(url) {
		...
		if (!category) throw new ScriptException("tvQuran category playlist not found");
		return category;
	}
	function getPlaylistCollectionFromUrl(url) {
		const match = url.match(REGEX.COLLECTION_PLAYLIST);
		if (match) return getCollectionFromUrl(collectionChannelUrl(match[1], match[2]));
	...
	function reciterContentVideos(reciter, query, filters = null) {
		const q = normalize(query);
		return (state.reciterContentsCache[reciter.id] ?? parseReciterProfile(callHtml(reciter.pageUrl || reciterProfileUrl(reciter.id, reciter.slug)), reciter.pageUrl).recitations).filter((recitation) => recitationMatchesQuery(recitation, q)).filter((recitation) => recitationMatchesFilters(recitation, filters)).map(recitationToVideo);
	}
	function collectionContentVideos(collection, query, filters = null) {
		const q = normalize(query);
		return collection.recitations.filter((recitation) => recitationMatchesQuery(recitation, q)).filter((recitation) => recitationMatchesFilters(recitation, filters)).map(recitationToVideo);
	}
	...
	function reciterSearchUrl(query, page = 1) {
		const url = normalize(query) ? `${BASE_URL}/${language()}/quran/recitations/reciters-list?json=1&view=search&query=${encodeURIComponent(query)}` : `${BASE_URL}/${language()}/quran/recitations/reciters-list?json=1&view=${encodeURIComponent(reciterSortView())}`;
	...
	function searchSelectionsPage(query, order, filters, page = 1) {
		...
		if (categoryIds.length) {
			...
			return {
				items: selections.map(selectionToVideo),
				hasMore
			};
		}
		if (q) {
			...
			const parsed = parseSelections(html);
			return {
				items: parsed.filter((selection) => selectionMatches(selection, q) && selectionMatchesFilters(selection, filters)).map(selectionToVideo),
				hasMore: pageHasMore(html, parsed)
			};
		}
		const html = callHtml(categoryUrl((CATEGORIES[Number(_settings.homeCategory ?? 0)] ?? CATEGORIES[0]).id, page, view));
		const parsed = parseSelections(html);
		return {
	...
	function searchRecitationsPage(query, page = 1, filters = null) {
		...
		if (selectedReciters.length) {
			...
			const results = [];
			for (const reciter of reciters) results.push(...reciterContentVideos(reciter, q, filters));
			return {
				items: results,
				hasMore: false
			};
		...
		if (q && !pageResult.reciters.length) pageResult = reciterSearchPage("", page);
		const results = [];
		for (const reciter of pageResult.reciters.slice(0, RECITATION_SCAN_PER_PAGE)) results.push(...reciterContentVideos(reciter, q, filters));
		return {
			items: results,
	...
	function searchTvVideosPage(query, videoType, page = 1, filters = null) {
		const q = normalize(query);
		const html = callHtml(videoCategoryUrl((VIDEO_CATEGORIES.find((item) => item.type === videoType) ?? VIDEO_CATEGORIES[0]).type, page));
		const videos = parseTvVideos(html, videoType);
		return {
			items: videos.filter((video) => !q || normalize(video.title).indexOf(q) >= 0 || normalize(video.description).indexOf(q) >= 0).filter((video) => tvVideoMatchesFilters(video, filters)).map(tvVideoToNested),
	...
	function channelSearchPage(kind, value, query, order, filters, page = 1) {
		if (kind === "category") {
			...
			return {
				videos: selections.filter((selection) => selectionMatches(selection, q) && selectionMatchesFilters(selection, filters)).map(selectionToVideo),
	...
	function resolveReciterFilter(value) {
		...
		if (directId && state.reciterCache[String(directId)]) return state.reciterCache[String(directId)];
		...
		return reciterSearchResults(textValue).find((reciter) => normalize(reciter.name).indexOf(q) >= 0 || q.indexOf(normalize(reciter.name)) >= 0) ?? null;
	}
	function contentFilter() {
		return {
			...
			filters: [
				...
				{
					id: text("prayerVideos"),
					name: text("prayerVideos"),
					value: "prayer-videos"
				},
				{
					id: text("liveVideos"),
					name: text("liveVideos"),
					value: "live-videos"
				}
			]
		};
	...
	function hasSearchFilters(filters) {
		return selectedFilterValues(filters, "content").length > 0 || selectedFilterValues(filters, "category").length > 0 || selectedFilterValues(filters, "reciter").length > 0 || selectedFilterValues(filters, "surah").length > 0;
	}
	function contentAllowed(filters, contentType) {
		const selected = selectedFilterValues(filters, "content");
		if (selected.length) return selected.indexOf(contentType) >= 0;
		if (selectedFilterValues(filters, "category").length) return contentType === "selections";
		return true;
	}
	function selectedFilterValues(filters, id) {
		if (!filters) return [];
		let value = filters[id];
		if (value === void 0 && filters.get) value = filters.get(id);
		if (value === void 0 || value === null) return [];
		if (!Array.isArray(value)) value = [value];
		return value.map(filterValue).filter(Boolean);
	}
	function filterValue(value) {
		if (typeof value === "object" && value !== null) return String(value.value ?? value.id ?? "");
		return String(value);
	}
	...
	function surahIdFromFilter(value) {
		...
		return Number(SURAHS.find((surah) => surahFilterLabel(surah) === textValue || surahName(surah) === textValue || Object.values(surah.name).indexOf(textValue) >= 0)?.id);
	}
	function selectionMatches(selection, query) {
		return !query || normalize(selection.title).indexOf(query) >= 0 || normalize(selection.author).indexOf(query) >= 0 || normalize(selection.category).indexOf(query) >= 0;
	}
	function selectionMatchesFilters(selection, filters) {
		...
		return (!selectedReciters.length || selectedReciters.some((reciter) => normalize(selection.author).indexOf(normalize(reciter)) >= 0)) && (!selectedSurahIds.length || selectedSurahIds.some((surahId) => textMentionsSurah(`${selection.title} ${selection.category}`, surahId)));
	}
	function recitationMatchesFilters(recitation, filters) {
		...
		const selectedSurahIds = selectedFilterValues(filters, "surah").map(surahIdFromFilter).filter(Boolean);
		return (!selectedReciterIds.length && !selectedReciters.length || selectedReciterIds.indexOf(Number(recitation.reciter.id)) >= 0 || selectedReciters.indexOf(normalize(recitation.reciter.name)) >= 0) && (!selectedSurahIds.length || selectedSurahIds.indexOf(Number(recitation.surahNumber)) >= 0 || selectedSurahIds.some((surahId) => textMentionsSurah(recitation.title, surahId)));
	}
	function tvVideoMatchesFilters(video, filters) {
		const selectedReciters = selectedFilterValues(filters, "reciter").map(normalize);
		const selectedSurahIds = selectedFilterValues(filters, "surah").map(surahIdFromFilter).filter(Boolean);
		const searchableText = `${video.title} ${video.description}`;
		return (!selectedReciters.length || selectedReciters.some((reciter) => normalize(searchableText).indexOf(reciter) >= 0)) && (!selectedSurahIds.length || selectedSurahIds.some((surahId) => textMentionsSurah(searchableText, surahId)));
	}
	function recitationMatchesQuery(recitation, query) {
		return !query || normalize(recitation.reciter.name).indexOf(query) >= 0 || normalize(recitation.title).indexOf(query) >= 0 || normalize(recitation.collectionTitle).indexOf(query) >= 0 || String(recitation.surahNumber || "").indexOf(query) >= 0;
	}
	...
	function textMentionsSurah(value, surahId) {
		...
		return Boolean(surah) && (numberPattern.test(normalizedValue) || Object.values(surah.name).some((name) => normalizedValue.indexOf(normalize(name)) >= 0));
	}
	...
	function selectionViewFromOrder(order) {
		if (order === Type.Order.Chronological) return "new";
		if (order === Type.Order.Popularity) return "most_listened";
	...
	function categoryChannelMetadata(kind, category, enrich) {
		...
		if (state.categoryMetadataCache?.[cacheKey]) return {
			...fallback,
			...state.categoryMetadataCache[cacheKey]
		};
	...
	var extractFirst = extract_first;
	var lastMatch = last_match;
	var cleanText = clean_text;
	...
	function extractYoutubeId(value) {
		return extractFirst(value, /youtube\.com\/embed\/([a-zA-Z0-9_-]+)/) ?? extractFirst(value, /youtu\.be\/([a-zA-Z0-9_-]+)/) ?? extractFirst(value, /i\.ytimg\.com\/vi\/([a-zA-Z0-9_-]+)\//);
	}
	...
	function logIfTesting(msg) {
		if (typeof IS_TESTING !== "undefined" && IS_TESTING) log(msg);
	}
	log("LOADED");
