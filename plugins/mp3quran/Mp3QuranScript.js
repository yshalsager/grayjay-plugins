...
function contentThumbnails(url) {
  return new Thumbnails([new Thumbnail(url || iconUrl(), 0)])
}
...
function trackToVideo(reciter, moshaf, surahId) {
  ...
  const details = new PlatformVideoDetails({
    id: new PlatformID(PLATFORM, `${reciter.id}-${moshaf.id}-${surahId}`, _config.id),
    name,
...
function tafsirToVideo(item) {
  ...
  const details = new PlatformVideoDetails({
    id: new PlatformID(PLATFORM, `tafsir-${item.tafsir_id}-${item.id}`, _config.id),
    name: item.name,
    thumbnails: staticThumbnails(),
    author: new PlatformAuthorLink(
      new PlatformID(PLATFORM, `tafsir-${item.tafsir_id}`, _config.id),
      item.tafsir_name,
      'https://mp3quran.net',
      iconUrl()
    ),
...
function radioToVideo(radio) {
  return new PlatformVideoDetails({
    id: new PlatformID(PLATFORM, `radio-${radio.id}`, _config.id),
    name: radio.name,
    thumbnails: staticThumbnails(),
    author: new PlatformAuthorLink(new PlatformID(PLATFORM, 'radio', _config.id), 'MP3Quran Radio', 'https://mp3quran.net', iconUrl()),
    uploadDate: 0,
    duration: 0,
    viewCount: 0,
    isLive: true,
...
function mp3VideoToVideo(video) {
  ...
  const details = new PlatformVideoDetails({
    id: new PlatformID(PLATFORM, `video-${video.reciter_id}-${video.id}`, _config.id),
    name: title,
    thumbnails: new Thumbnails([new Thumbnail(video.video_thumb_url || iconUrl(), 0)]),
    author: new PlatformAuthorLink(
      new PlatformID(PLATFORM, String(video.reciter_id), _config.id),
      video.reciter_name,
      reciterUrl(video.reciter_id),
      iconUrl()
    ),
    uploadDate: 0,
    duration: 0,
...
function liveTvToVideo(liveTv) {
  ...
  return new PlatformVideoDetails({
    id: new PlatformID(PLATFORM, `live-tv-${liveTv.id}`, _config.id),
    name: liveTv.name,
    thumbnails: staticThumbnails(),
    author: new PlatformAuthorLink(
      new PlatformID(PLATFORM, 'live-tv', _config.id),
      'MP3Quran Live TV',
      'https://mp3quran.net/live',
      iconUrl()
    ),
    uploadDate: 0,
    duration: 0,
    ...
    video: new UnMuxVideoSourceDescriptor([hls], []),
    live: hls,
...
function reciterToAuthor(reciter) {
  return new PlatformAuthorLink(
    new PlatformID(PLATFORM, String(reciter.id), _config.id),
    reciter.name,
    reciterUrl(reciter.id),
    reciterThumbnailUrl(reciter.id) || iconUrl()
  )
}
...
function reciterToChannel(reciter) {
  return new PlatformChannel({
    id: new PlatformID(PLATFORM, String(reciter.id), _config.id),
    name: reciter.name,
...
class ArrayChannelPager extends ChannelPager {
  constructor(items, limit, offset = 0) {
    const results = items.slice(offset, offset + limit)
    super(results, offset + limit < items.length, {
      items,
  ...
  nextPage() {
    const next = new ArrayChannelPager(this.context.items, this.context.limit, this.context.offset)
    this.results = next.results
    this.hasMore = next.hasMore
    this.context = next.context
    return this
...
function moshafToPlaylist(reciter, moshaf) {
  ...
  return new PlatformPlaylist({
    id: new PlatformID(PLATFORM, `playlist-moshaf-${reciter.id}-${moshaf.id}`, _config.id),
...
function tafsirToPlaylist(tafsir) {
  ...
  return new PlatformPlaylist({
    id: new PlatformID(PLATFORM, `playlist-tafsir-${tafsir.id}`, _config.id),
    name: tafsir.name,
    thumbnails: staticThumbnails(),
...
function videoTypeToPlaylist(videoType) {
  ...
  return new PlatformPlaylist({
    id: new PlatformID(PLATFORM, `playlist-video-type-${videoType.id}`, _config.id),
...
function mp3QuranAuthor(id, name) {
  return new PlatformAuthorLink(new PlatformID(PLATFORM, id, _config.id), name, 'https://mp3quran.net', iconUrl())
}
