(function() {
	...
	function video_source_descriptor({ name, url, width = 0, height = 0, container = "video/mp4", codec = "h264", duration = 0, bitrate = 0 }) {
		return new VideoSourceDescriptor([new VideoUrlSource({
			name,
			width,
			height,
			container,
	...
	function hls_source({ name, url, language = "", duration = -1, priority = true }) {
	...
	function hls_source_descriptor(source) {
	...
	var PLATFORM = "MP3Quran";
	var API_BASE = "https://www.mp3quran.net/api/v3";
	...
	source.getHome = function() {
		...
		if (homeMode === HOME_MODES.RECITATIONS) return new ArrayVideoPager(buildCompleteTracks(), DEFAULT_LIMIT);
		if (homeMode === HOME_MODES.LIVE_TV) return new ArrayVideoPager(state.liveTv.map(liveTvToVideo), DEFAULT_LIMIT);
		if (homeMode === HOME_MODES.RECENT_READS) return new ArrayVideoPager(buildRecentReadTracks(), DEFAULT_LIMIT);
		if (homeMode === HOME_MODES.TAFSIR) return new ArrayVideoPager(state.tafsirItems.map(tafsirToVideo), DEFAULT_LIMIT);
	...
	source.searchSuggestions = function(query) {
		...
		]).filter((name) => normalize(name).indexOf(q) >= 0).slice(0, 10);
	};
	source.search = function(query, type, order, filters) {
		...
		const results = [];
		if (contentAllowed(filters, "live-tv")) results.push(...state.liveTv.filter((tv) => liveTvMatches(tv, q)).map(liveTvToVideo));
		if (contentAllowed(filters, "radios")) results.push(...state.radios.filter((radio) => radioMatches(radio, q)).map(radioToVideo));
	...
	source.searchChannels = function(query) {
		ensureCatalog();
		const q = normalize(query);
		return new ArrayChannelPager(state.reciters.filter((reciter) => !q || normalize(reciter.name).indexOf(q) >= 0).map(reciterToChannel), DEFAULT_LIMIT);
	};
	...
	source.getChannelPlaylists = function(url) {
		ensureCatalog();
		if (REGEX.ROOT_CHANNEL.test(url)) return new ArrayPlaylistPager(mp3Playlists("", null), DEFAULT_LIMIT);
		const reciter = getReciterFromUrl(url);
		return new ArrayPlaylistPager((reciter.moshaf ?? []).map((moshaf) => moshafToPlaylist(reciter, moshaf)), DEFAULT_LIMIT);
	};
	...
	source.peekChannelContents = function(url, type) {
		...
		if (peekType === normalize(text("videos")) || peekType === "videos") return mp3Videos("", { reciter: reciter.name }).slice(0, 6);
		const moshaf = primaryMoshaf(reciter);
		if (!moshaf) return [];
		return parseSurahList(moshaf.surah_list).slice(0, 6).map((surahId) => trackToVideo(reciter, moshaf, surahId));
	};
	source.searchChannelContents = function(url, query, type, order, filters) {
		ensureCatalog();
		if (REGEX.ROOT_CHANNEL.test(url)) return source.search(query, type, order, filters);
		const q = normalize(query);
		return new ArrayVideoPager(reciterTracks(getReciterFromUrl(url), q, filters), DEFAULT_LIMIT);
	};
	...
	source.getContentChapters = function(url) {
		...
		try {
			...
			return trackTiming(track.moshaf, track.surahId).filter((timing) => Number(timing.end_time) > Number(timing.start_time)).map((timing) => ({
				name: chapterLabel(track.surahId, timing, versesByNumber),
				timeStart: Number(timing.start_time) / 1e3,
				timeEnd: Number(timing.end_time) / 1e3,
	...
	function rebuildIndexes() {
		...
		for (const reciter of state.reciters ?? []) state.reciterById[String(reciter.id)] = reciter;
		for (const surah of state.suwar ?? []) state.surahById[Number(surah.id)] = surah;
		for (const radio of state.radios ?? []) state.radioById[String(radio.id)] = radio;
		for (const liveTv of state.liveTv ?? []) state.liveTvById[String(liveTv.id)] = liveTv;
		for (const tafsir of state.tafsirItems ?? []) state.tafsirByKey[tafsirKey(tafsir.tafsir_id, tafsir.id)] = tafsir;
		for (const video of state.videos ?? []) state.videoByKey[videoKey(video.reciter_id, video.id)] = video;
		for (const videoType of state.videoTypes ?? []) state.videoTypeById[String(videoType.id)] = videoType;
		for (const read of state.timingReads ?? []) state.timingReadByFolder[normalizeFolderUrl(read.folder_url)] = read;
	}
	function loadTafsir(language) {
		...
		for (const tafsir of catalog) {
			...
			for (const item of details?.soar ?? []) items.push({
				...item,
	...
	function flattenVideos(groups) {
		const videos = [];
		for (const group of groups ?? []) for (const video of group.videos ?? []) videos.push({
			...video,
			reciter_id: group.id,
	...
	function buildRecentReadTracks() {
		const tracks = [];
		for (const reciter of state.recentReads ?? []) for (const moshaf of reciter.moshaf ?? []) {
			const firstSurah = parseSurahList(moshaf.surah_list)[0];
			if (firstSurah) tracks.push(trackToVideo(reciter, moshaf, firstSurah));
		}
		return tracks;
	}
	...
	function tracksFromReciters(reciters, query, filters) {
		...
		for (const reciter of reciters) {
			if (selectedReciterIds.length && selectedReciterIds.indexOf(Number(reciter.id)) < 0) continue;
			for (const moshaf of reciter.moshaf ?? []) {
				if (selectedRiwayahIds.length && selectedRiwayahIds.indexOf(Number(moshaf.rewaya_id)) < 0) continue;
				const moshafSurahIds = parseSurahList(moshaf.surah_list);
				const targetSurahIds = selectedSurahIds.length ? selectedSurahIds.filter((surahId) => moshafSurahIds.indexOf(surahId) >= 0) : moshafSurahIds;
				for (const surahId of targetSurahIds) {
					const surah = state.surahById[surahId];
					if (!q || trackMatches(reciter, moshaf, surah, q)) tracks.push(trackToVideo(reciter, moshaf, surahId));
				}
			}
		}
		return tracks;
	...
	function tafsirTracks(query, filters) {
		...
		return (state.tafsirItems ?? []).filter((item) => !selectedTafsirIds.length || selectedTafsirIds.indexOf(Number(item.tafsir_id)) >= 0).filter((item) => !selectedSurahIds.length || selectedSurahIds.indexOf(Number(item.sura_id)) >= 0).filter((item) => !q || tafsirMatches(item, q)).map(tafsirToVideo);
	}
	...
	function trackDescription(reciter, moshaf, surah, audioUrl) {
		...
		if (reciter.recent_date) lines.push(`${text("updated")}: ${reciter.recent_date}`);
		lines.push(`${text("source")}: ${audioUrl}`);
		return lines.join("\n");
	}
	function trackSubtitles(moshaf, surahId) {
		if (!timingReadForMoshaf(moshaf)) return [];
		try {
			return [{
				name: text("quranText"),
				language: "ar",
	...
	function buildTrackWebVtt(moshaf, surahId) {
		...
		return `WEBVTT\n\n${trackTiming(moshaf, surahId).filter((timing) => versesByNumber[Number(timing.ayah)]).filter((timing) => Number(timing.end_time) > Number(timing.start_time)).map((timing) => `${vttTimestamp(timing.start_time)} --> ${vttTimestamp(timing.end_time)}\n${versesByNumber[Number(timing.ayah)].text.trim()}`).join("\n\n")}\n`;
	}
	function chapterLabel(surahId, timing, versesByNumber) {
		const ayah = Number(timing.ayah);
		if (!ayah) return text("opening");
	...
	function surahText(surahId) {
		...
		if (!quranTextBySurah[key]) quranTextBySurah[key] = callJson(`${QURAN_TEXT_BASE}/${surahId}.min.json`).chapter ?? [];
		return quranTextBySurah[key];
	}
	function versesByAyah(verses) {
		const byAyah = {};
		for (const verse of verses ?? []) byAyah[Number(verse.verse)] = verse;
		return byAyah;
	}
	...
	function tafsirToVideo(item) {
		...
		details.getContentRecommendations = function() {
			return new ArrayVideoPager((state.tafsirItems ?? []).filter((other) => Number(other.sura_id) === Number(item.sura_id) && tafsirKey(other.tafsir_id, other.id) !== tafsirKey(item.tafsir_id, item.id)).map(tafsirToVideo), DEFAULT_LIMIT);
		};
		return details;
	}
	...
	function relatedMp3Videos(current) {
		return (state.videos ?? []).filter((video) => videoKey(video.reciter_id, video.id) !== videoKey(current.reciter_id, current.id)).filter((video) => Number(video.reciter_id) === Number(current.reciter_id) || Number(video.video_type) === Number(current.video_type)).sort((a, b) => videoRelationScore(current, b) - videoRelationScore(current, a));
	}
	function videoRelationScore(current, video) {
		return (Number(video.reciter_id) === Number(current.reciter_id) ? 2 : 0) + (Number(video.video_type) === Number(current.video_type) ? 1 : 0);
	}
	...
	function mp3Playlists(query, filters) {
		...
		if (contentAllowed(filters, "tracks")) {
			for (const reciter of state.reciters ?? []) for (const moshaf of reciter.moshaf ?? []) if (moshafMatchesFilters(reciter, moshaf, filters)) playlists.push(moshafToPlaylist(reciter, moshaf));
		}
		if (contentAllowed(filters, "tafsir")) playlists.push(...tafsirPlaylists(filters));
		if (contentAllowed(filters, "videos")) playlists.push(...videoTypePlaylists(filters));
	...
	function moshafToPlaylistDetails(reciter, moshaf) {
		return grayjay.playlist_details(`playlist-moshaf-${reciter.id}-${moshaf.id}`, {
			...
			videoCount: parseSurahList(moshaf.surah_list).length,
			thumbnail: reciterThumbnailUrl(reciter.id) || iconUrl(),
			contents: new ArrayVideoPager(parseSurahList(moshaf.surah_list).map((surahId) => trackToVideo(reciter, moshaf, surahId)), DEFAULT_LIMIT)
		});
	}
	function tafsirPlaylists(filters) {
		const selectedTafsirIds = selectedFilterValues(filters, "tafsir-source").map(tafsirIdFromFilter).filter(Boolean);
		const selectedSurahIds = selectedFilterValues(filters, "surah").map(surahIdFromFilter).filter(Boolean);
		return (state.tafasir ?? []).filter((tafsir) => !selectedTafsirIds.length || selectedTafsirIds.indexOf(Number(tafsir.id)) >= 0).filter((tafsir) => !selectedSurahIds.length || (state.tafsirItems ?? []).some((item) => Number(item.tafsir_id) === Number(tafsir.id) && selectedSurahIds.indexOf(Number(item.sura_id)) >= 0)).map(tafsirToPlaylist);
	}
	...
	function videoTypePlaylists(filters) {
		const selectedVideoTypeIds = selectedFilterValues(filters, "video-type").map(videoTypeIdFromFilter).filter(Boolean);
		const selectedReciterIds = selectedFilterValues(filters, "reciter").map(reciterIdFromFilter).filter(Boolean);
		return (state.videoTypes ?? []).filter((videoType) => !selectedVideoTypeIds.length || selectedVideoTypeIds.indexOf(Number(videoType.id)) >= 0).filter((videoType) => !selectedReciterIds.length || (state.videos ?? []).some((video) => Number(video.video_type) === Number(videoType.id) && selectedReciterIds.indexOf(Number(video.reciter_id)) >= 0)).map(videoTypeToPlaylist);
	}
	...
	function getMp3Playlist(url) {
		...
		if (match) {
			...
			if (!reciter || !moshaf) throw new ScriptException("Moshaf playlist not found");
		...
		if (match) {
			...
			if (!tafsir) throw new ScriptException("Tafsir playlist not found");
		...
		if (match) {
			...
			if (!videoType) throw new ScriptException("Video playlist not found");
	...
	function moshafMatchesFilters(reciter, moshaf, filters) {
		...
		const selectedRiwayahIds = selectedFilterValues(filters, "riwayah").map(riwayahIdFromFilter).filter(Boolean);
		const selectedSurahIds = selectedFilterValues(filters, "surah").map(surahIdFromFilter).filter(Boolean);
		return (!selectedReciterIds.length || selectedReciterIds.indexOf(Number(reciter.id)) >= 0) && (!selectedRiwayahIds.length || selectedRiwayahIds.indexOf(Number(moshaf.rewaya_id)) >= 0) && (!selectedSurahIds.length || selectedSurahIds.some((surahId) => parseSurahList(moshaf.surah_list).indexOf(surahId) >= 0));
	}
	...
	function trackFromUrl(url) {
		...
		const moshaf = reciter?.moshaf?.find((m) => String(m.id) === match[2]);
		const surahId = Number(match[3]);
		if (!reciter || !moshaf || !state.surahById[surahId]) return null;
		return {
			reciter,
			moshaf,
			surahId
	...
	function trackMatches(reciter, moshaf, surah, query) {
		return normalize(reciter.name).indexOf(query) >= 0 || normalize(moshaf.name).indexOf(query) >= 0 || normalize(surah?.name).indexOf(query) >= 0 || String(surah?.id ?? "").indexOf(query) >= 0;
	}
	function tafsirMatches(item, query) {
		const surah = state.surahById[Number(item.sura_id)];
		return normalize(item.name).indexOf(query) >= 0 || normalize(item.tafsir_name).indexOf(query) >= 0 || normalize(surah?.name).indexOf(query) >= 0 || String(item.sura_id ?? "").indexOf(query) >= 0;
	}
	...
	function mp3VideoMatches(video, query) {
		return normalize(video.reciter_name).indexOf(query) >= 0 || normalize(getVideoTypeName(video.video_type)).indexOf(query) >= 0 || String(video.id ?? "").indexOf(query) >= 0;
	}
	function hasSearchFilters(filters) {
		return selectedFilterValues(filters, "content").length > 0 || selectedFilterValues(filters, "reciter").length > 0 || selectedFilterValues(filters, "surah").length > 0 || selectedFilterValues(filters, "riwayah").length > 0 || selectedFilterValues(filters, "video-type").length > 0 || selectedFilterValues(filters, "tafsir-source").length > 0;
	}
	function contentAllowed(filters, contentType) {
		...
		if (selectedFilterValues(filters, "riwayah").length) return contentType === "tracks";
		if (selectedFilterValues(filters, "reciter").length) return contentType === "tracks" || contentType === "videos";
		if (selectedFilterValues(filters, "video-type").length) return contentType === "videos";
		if (selectedFilterValues(filters, "tafsir-source").length) return contentType === "tafsir";
		if (selectedFilterValues(filters, "surah").length) return contentType === "tracks" || contentType === "tafsir";
		return true;
	}
	...
	function reciterThumbnailUrl(reciterId) {
		return (state.videos ?? []).find((video) => String(video.reciter_id) === String(reciterId) && video.video_thumb_url)?.video_thumb_url ?? null;
	}
	...
	function vttTimestamp(ms) {
		...
		const milliseconds = Math.floor(value % 1e3);
	...
	function sortByName(items) {
		return [...items ?? []].sort((a, b) => String(a.name ?? "").localeCompare(String(b.name ?? ""), state.language || void 0));
	}
