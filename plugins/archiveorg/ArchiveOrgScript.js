(function() {
	...
	var PLATFORM = "Archive.org";
	var BASE_URL = "https://archive.org";
	var SEARCH_URL = `${BASE_URL}/advancedsearch.php`;
	var DEFAULT_LIMIT = 24;
	var PLAYLIST_LIMIT = 8;
	var SEARCH_COUNT = 100;
	var DEFAULT_ICON = "https://archive.org/images/glogo.png";
	var MEDIA = {
		BOTH: 0,
		AUDIO: 1,
		VIDEO: 2
	...
	source.search = function(query, type, order, filters) {
		if (!normalize(query) && !hasSearchFilters(filters)) return new ArrayVideoPager([], DEFAULT_LIMIT);
	...
	source.searchPlaylists = function(query, type, order, filters) {
		if (!normalize(query) && !hasSearchFilters(filters)) return new EmptyPlaylistPager();
	...
	source.peekChannelContents = function(url, _type) {
		return new ArchiveSearchPager("", mediaType(), defaultSort(), null, null, 0, false, creatorFromUrl(url)).results.slice(0, 6);
	};
	...
	source.getContentDetails = function(url) {
		...
		if (direct) return fileToVideo(direct.item, direct.file, url);
		const item = getItemFromUrl(url);
		const file = firstPlayableFile(item, mediaFromUrl(url));
		if (!file) throw new ScriptException("Archive.org item has no playable audio or video files");
		return fileToVideo(item, file, url);
	};
	var ArchiveSearchPager = class ArchiveSearchPager extends VideoPager {
		constructor(query, media, order, pageNumber = 1, items = null, offset = 0, sourceHasMore = false, creator = null) {
			const page = items ? {
				items,
				hasMore: sourceHasMore,
				nextPage: pageNumber
			} : searchArchive(query, media, order, pageNumber, creator);
		...
		nextPage() {
			...
			const next = new ArchiveSearchPager(this.context.query, this.context.media, this.context.order, this.context.page, remainingItems, offset, remainingItems ? this.context.sourceHasMore : false, this.context.creator);
			return apply_pager_state(this, next);
		}
	};
	var ArchivePlaylistSearchPager = class ArchivePlaylistSearchPager extends PlaylistPager {
		constructor(query, media, order, pageNumber = 1, items = null, offset = 0, sourceHasMore = false, creator = null) {
			const page = items ? {
				items,
				hasMore: sourceHasMore,
		...
		nextPage() {
			const remainingItems = this.context.offset < this.context.items.length ? this.context.items : null;
			const offset = remainingItems ? this.context.offset : 0;
			const next = new ArchivePlaylistSearchPager(this.context.query, this.context.media, this.context.order, this.context.page, remainingItems, offset, remainingItems ? this.context.sourceHasMore : false, this.context.creator);
			return apply_pager_state(this, next);
		}
	};
	var ArrayVideoPager = class extends array_pager_class(VideoPager) {
		constructor(items, limit = DEFAULT_LIMIT, offset = 0) {
			super(items, limit, offset);
		}
	};
	var EmptyChannelPager = empty_pager_class(ChannelPager);
	var EmptyPlaylistPager = empty_pager_class(PlaylistPager);
	var ArchiveCreatorPager = class ArchiveCreatorPager extends ChannelPager {
	...
	function searchArchive(query, media, order, pageNumber, creator = null) {
		...
		const response = callJson(`${SEARCH_URL}?${query_string(params)}`).response ?? {};
		const items = response.docs ?? [];
		const page = Number(pageNumber || 1);
		return {
			items: sort ? items : rankSearchItems(items, query),
			nextPage: page + 1,
	...
	function searchQuery(query, media, creator = null) {
		...
		if (creator) parts.push(creatorSearchQuery(creator));
		if (q) parts.push(userSearchQuery(q));
		if (media === MEDIA.AUDIO) parts.push("mediatype:audio");
		else if (media === MEDIA.VIDEO) parts.push("mediatype:movies");
		else parts.push("(mediatype:audio OR mediatype:movies)");
		return parts.join(" AND ");
	}
	function userSearchQuery(query) {
		const terms = searchTerms(query);
		const clauses = [`"${escapeSearch(query)}"`, escapeSearch(query)];
		if (terms.length > 1) clauses.push(terms[terms.length - 1]);
	...
	function rankSearchItems(items, query) {
		...
		return items.map((item, index) => ({
			item,
			index,
			score: searchScore(item, query, terms)
	...
	function searchScore(item, query, terms) {
		...
		let score = 0;
		if (normalizedQuery && title.indexOf(normalizedQuery) >= 0) score += 1e3;
	...
	function itemsToPlaylists(items, media) {
		const playlists = [];
		for (const summary of items ?? []) try {
			const item = getItem(summary.identifier, summary);
			if (firstPlayableFile(item, media)) playlists.push(itemToPlaylist(item, media));
		} catch (e) {
	...
	function getItem(identifier, summary = null) {
		...
		if (!id) throw new ScriptException("Archive.org item not found");
		if (!state.itemCache[id]?.files?.length) {
			const item = callJson(`${BASE_URL}/metadata/${encodeURIComponent(id)}`);
			if (!item?.files?.length && !summary) throw new ScriptException("Archive.org item not found");
			state.itemCache[id] = normalizeItem(item, id, summary);
		} else if (summary && !state.itemCache[id].summary) state.itemCache[id].summary = summary;
	...
	function getItemFromUrl(url) {
		const match = url.match(REGEX.ITEM) ?? url.match(REGEX.PLAYLIST) ?? url.match(REGEX.CONTENT) ?? url.match(REGEX.WEB_DETAILS) ?? url.match(REGEX.WEB_DOWNLOAD);
		if (!match?.[1]) throw new ScriptException("Archive.org item not found");
	...
	function fileFromUrl(url) {
		...
		if (!file) throw new ScriptException("Archive.org file not found");
		return {
			item,
			file
		};
	...
	function firstPlayableFile(item, media = MEDIA.BOTH) {
		...
		let best = null;
		for (const file of item.files ?? []) if (isPlayableFile(file, preferredMedia) && (!best || fileSort(file, best) < 0)) best = file;
		return best;
	}
	function itemMedia(item, media = MEDIA.BOTH) {
		if (media !== MEDIA.BOTH) return media;
		const mediatype = normalize(item?.metadata?.mediatype || item?.summary?.mediatype);
		if (mediatype === "movies" || mediatype === "movie") return MEDIA.VIDEO;
	...
	function isPlayableFile(file, media = MEDIA.BOTH) {
		if (isSkippedFile(file)) return false;
	...
	function isSkippedFile(file) {
		...
		return !name || SKIPPED_FORMATS.some((skipped) => format.indexOf(skipped) >= 0) || /\.(xml|json|txt|pdf|torrent|sqlite|srt|vtt|jpg|jpeg|png|gif|zip|rar|7z)$/i.test(name);
	}
	function fileKind(file) {
		const name = String(file.name ?? "");
		const format = String(file.format ?? "");
		if (PLAYABLE_AUDIO_FORMATS.some((value) => format.indexOf(value) >= 0) || /\.(mp3|ogg|oga|flac)$/i.test(name)) return MEDIA.AUDIO;
		if (PLAYABLE_VIDEO_FORMATS.some((value) => format.indexOf(value) >= 0) || /\.(mp4|m4v|webm|ogv)$/i.test(name)) return MEDIA.VIDEO;
		return 0;
	}
	...
	function summariesToCreatorChannels(items) {
		const channels = [];
		const seen = {};
		for (const item of items ?? []) for (const creator of creatorsFromSummary(item)) {
	...
	function creatorsFromSummary(summary) {
		return (Array.isArray(summary?.creator) ? summary.creator : summary?.creator ? [summary.creator] : []).map(cleanText).filter(Boolean);
	}
	function fileToVideo(item, file, urlOverride = null) {
		...
		details.getContentRecommendations = function() {
			return new ArrayVideoPager(playableFiles(item).filter((candidate) => candidate.name !== file.name).map((candidate) => fileToVideo(item, candidate)), DEFAULT_LIMIT);
		};
		return details;
	}
	function sourceDescriptor(file, url, kind, fallbackUrl = null) {
		if (kind === MEDIA.AUDIO) return audio_sources_descriptor([url, fallbackUrl].filter((value, index, list) => value && list.indexOf(value) === index).map((sourceUrl) => ({
	...
	function itemToPlaylistDetails(item, media = MEDIA.BOTH) {
		...
		return grayjay.playlist_details(`playlist-${itemIdentifier(item)}`, {
			...
			contents: new ArrayVideoPager(playableFiles(item, media).map((file) => fileToVideo(item, file)), DEFAULT_LIMIT)
		});
	}
	function itemAuthor(item) {
		const creator = arrayText((item.metadata ?? {}).creator || item.summary?.creator) || "Internet Archive";
	...
	function creatorId(creator) {
		return normalize(creator).replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "internet-archive";
	}
	...
	function creatorFromUrl(url) {
		...
		if (!match?.[1]) throw new ScriptException("Archive.org creator not found");
	...
	function cacheSummary(summary) {
		const identifier = summaryIdentifier(summary);
		if (!identifier || state.itemCache[identifier]?.files?.length) return;
	...
	function itemIdentifier(item) {
		...
		if (!identifier || identifier === "undefined") throw new ScriptException("Archive.org item not found");
		return identifier;
	}
	...
	function selectedMedia(filters) {
		...
		if (filterValue(value) === "audio") return MEDIA.AUDIO;
		if (filterValue(value) === "video") return MEDIA.VIDEO;
		return mediaType();
	}
	...
	function defaultSort() {
		...
		if (sort === SORTS.DATE) return Type.Order.Chronological;
		if (sort === SORTS.DOWNLOADS) return Type.Order.Popularity;
		if (sort === SORTS.TITLE) return "Title";
		return null;
	}
	function sortParam(order) {
		if (order === Type.Order.Chronological) return "publicdate desc,identifier";
		if (order === Type.Order.Popularity) return "downloads desc,identifier";
		if (order === "Title") return "titleSorter asc,identifier";
		return null;
	}
	...
	function itemFileUrl(item, fileName) {
		...
		if (server && dir) return `https://${server}${encodePath(dir)}/${encodePath(fileName)}`;
		return fileUrl(itemIdentifier(item), fileName);
	}
	function itemServer(item) {
		return cleanText((item?.workable_servers ?? [])[0] || item?.d1 || item?.d2 || item?.server);
	}
	...
	function mediaFromUrl(url) {
		...
		if (match?.[2] === "audio") return MEDIA.AUDIO;
		if (match?.[2] === "video") return MEDIA.VIDEO;
		return MEDIA.BOTH;
	}
	function mediaSlug(media) {
		if (media === MEDIA.AUDIO) return "audio";
		if (media === MEDIA.VIDEO) return "video";
		return "both";
	}
	...
	function audioCodec(file) {
		const name = String(file.name ?? "").toLowerCase();
		if (name.endsWith(".ogg") || name.endsWith(".oga")) return "vorbis";
	...
	function videoCodec(file) {
		const name = String(file.name ?? "").toLowerCase();
		if (name.endsWith(".ogv")) return "theora";
	...
	function durationSeconds(value) {
		...
		return text.split(":").map(Number).reduce((total, part) => total * 60 + (part || 0), 0);
	}
	function bitrateBitsPerSecond(file) {
		...
		if (!Number.isFinite(size) || size <= 0 || !Number.isFinite(duration) || duration <= 0) return 0;
	...
	function searchTerms(value) {
		...
		return dedupeStrings(normalizeSearch(value).split(" ").filter((term) => term.length > 1 && stopWords.indexOf(term) < 0));
	}
